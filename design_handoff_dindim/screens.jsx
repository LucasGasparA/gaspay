...
function TabBar({ active, dark }) {
  ...
  return (
    <div style={{ display: 'flex', borderTop: `1px solid ${c.border}`, background: c.surface, paddingTop: 8, paddingBottom: 2, flexShrink: 0 }}>
      {tabs.map(t => (
        <div key={t} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4, paddingBottom: 6 }}>
          <div style={{ width: 20, height: 3, borderRadius: 2, background: t === active ? c.brand : 'transparent' }} />
          <div style={{ fontSize: 11, fontWeight: t === active ? fw.semibold : fw.regular, color: t === active ? c.brand : c.textTertiary }}>{t}</div>
        </div>
      ))}
    </div>
  );
...
function Field({ label, value, placeholder, secure, focused, error }) {
  const line = error ? colors.danger : focused ? colors.brand : colors.border;
  return (
    <div>
      <div style={{ fontSize: 13, color: colors.textSecondary, marginBottom: 8 }}>{label}</div>
      <div style={{ display: 'flex', alignItems: 'center', gap: space.sm, paddingBottom: 10, borderBottom: `2px solid ${line}` }}>
        <div style={{ flex: 1, fontSize: 17, color: value ? colors.text : colors.textTertiary, letterSpacing: secure ? '0.16em' : 'normal' }}>{value || placeholder}</div>
        {secure && <div style={{ fontSize: 13, fontWeight: fw.medium, color: colors.brand }}>ver</div>}
      </div>
...
function PrimaryButton({ label, loading }) {
  return (
    <div style={{ background: colors.brand, borderRadius: radius.pill, padding: '16px', textAlign: 'center', color: colors.onBrand, fontSize: 16, fontWeight: fw.semibold, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: space.sm }}>
      {loading && <div style={{ width: 14, height: 14, borderRadius: radius.pill, border: `2px solid ${colors.onBrand}`, borderTopColor: 'transparent' }} />}
      {label}
    </div>
  );
}
...
function TextLink({ label }) {
  return <div style={{ fontSize: 15, color: colors.textSecondary, textAlign: 'center' }}>{label}</div>;
}
...
function HomeScreenDark() {
  ...
  return (
    <AndroidDevice>
      <div style={{ height: '100%', display: 'flex', flexDirection: 'column', background: c.background, fontFamily: font }}>
        <div style={{ flex: 1, overflow: 'auto', padding: `${62 + space.md}px ${space.lg}px ${space.xl}px`, display: 'flex', flexDirection: 'column', gap: space.xl }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <div>
              <div style={{ fontSize: 24, fontWeight: fw.semibold, color: c.text }}>Oi, Larissa</div>
              <div style={{ fontSize: 13, color: c.textSecondary, marginTop: 2 }}>Segunda, 10 de agosto</div>
            </div>
            <div style={{ width: 44, height: 44, borderRadius: radius.pill, background: c.brandSubtle, color: c.brand, display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: fw.semibold, fontSize: 15 }}>LM</div>
          </div>
          <div>
            <div style={{ fontSize: 13, color: c.textSecondary }}>Saldo total</div>
            <div style={{ fontSize: 36, fontWeight: fw.semibold, color: c.text, marginTop: 4, fontVariantNumeric: 'tabular-nums' }}>{money(714235)}</div>
            <div style={{ display: 'flex', gap: space.lg, marginTop: space.sm }}>
              <div style={{ fontSize: 15, fontWeight: fw.medium, color: c.income }}>+{money(620000)}</div>
              <div style={{ fontSize: 15, fontWeight: fw.medium, color: c.text }}>-{money(394020)}</div>
            </div>
          </div>
          <div style={{ background: c.surface, borderRadius: radius.lg, padding: space.md, border: `1px solid ${c.border}` }}>
            <div style={{ fontSize: 15, fontWeight: fw.semibold, color: c.text, marginBottom: space.md }}>Como foram os 6 meses</div>
            <FlowChart data={monthlyFlow.map(d => ({ ...d }))} height={110} />
          </div>
          <div>
            <div style={{ fontSize: 17, fontWeight: fw.semibold, color: c.text, marginBottom: space.md }}>Meus limites do mês</div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: space.md }}>
              {budgets.slice(0, 3).map(b => (
                <div key={b.name}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 13, marginBottom: 6 }}>
                    <span style={{ color: c.text, fontWeight: fw.medium }}>{b.name}</span>
                    <span style={{ color: b.spent > b.limit ? c.danger : c.textSecondary }}>{money(b.spent)} de {money(b.limit)}</span>
                  </div>
                  <div style={{ height: 6, borderRadius: radius.pill, background: c.border, overflow: 'hidden' }}>
                    <div style={{ height: '100%', width: Math.min((b.spent / b.limit) * 100, 100) + '%', background: b.spent > b.limit ? c.danger : b.color }} />
                  </div>
                </div>
              ))}
            </div>
...
function LoginScreen() {
  return (
    <AndroidDevice>
      <div style={{ height: '100%', display: 'flex', flexDirection: 'column', background: colors.brand, fontFamily: font }}>
        <div style={{ padding: `${62 + space.lg}px ${space.lg}px ${space.xl}px` }}>
          <div style={{ width: 40, height: 40, borderRadius: 12, background: colors.onBrand, color: colors.brand, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 18, fontWeight: fw.semibold, marginBottom: space.lg }}>D</div>
          <div style={{ fontSize: 26, fontWeight: fw.semibold, color: colors.onBrand, lineHeight: 1.25 }}>Oi, bem-vinda<br/>de volta</div>
        </div>
        <div style={{ flex: 1, background: colors.surface, borderRadius: '28px 28px 0 0', display: 'flex', flexDirection: 'column', padding: `${space.xl}px ${space.lg}px ${space.xl}px`, overflow: 'auto' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: space.lg }}>
            ...
            <TextLink label="Entrar com Google" />
            <div style={{ textAlign: 'center', fontSize: 14, color: colors.textSecondary }}>
              Ainda não tem conta? <span style={{ color: colors.brand, fontWeight: fw.semibold }}>Criar agora</span>
            </div>
          </div>
        </div>
      </div>
...
function SignUpScreen() {
  return (
    <AndroidDevice>
      <div style={{ height: '100%', display: 'flex', flexDirection: 'column', background: colors.brand, fontFamily: font }}>
        <div style={{ padding: `${62 + space.lg}px ${space.lg}px ${space.xl}px` }}>
          <div style={{ width: 32, height: 32, borderRadius: 10, background: colors.onBrand, color: colors.brand, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 15, marginBottom: space.lg }}>‹</div>
          <div style={{ fontSize: 26, fontWeight: fw.semibold, color: colors.onBrand, lineHeight: 1.25 }}>Criar conta</div>
          <div style={{ fontSize: 14, color: colors.onBrand, opacity: 0.85, marginTop: 4 }}>Leva menos de um minuto.</div>
        </div>
        <div style={{ flex: 1, background: colors.surface, borderRadius: '28px 28px 0 0', display: 'flex', flexDirection: 'column', padding: `${space.xl}px ${space.lg}px ${space.xl}px`, overflow: 'auto', gap: space.xl }}>
          <Field label="Nome" value="Larissa Moraes" />
          <Field label="E-mail" value="larissa.moraes@email" error="E-mail incompleto" />
          <div>
            <Field label="Criar senha" placeholder="mínimo 8 caracteres" secure />
            <div style={{ display: 'flex', gap: 6, alignItems: 'center', marginTop: space.sm }}>
              {[colors.income, colors.income, colors.border].map((c, i) => (
                <div key={i} style={{ flex: 1, height: 4, borderRadius: radius.pill, background: c }} />
              ))}
              <span style={{ fontSize: 11, color: colors.textSecondary, marginLeft: 4 }}>boa</span>
            </div>
          </div>
...
function UnlockScreen() {
  return (
    <AndroidDevice>
      <div style={{ height: '100%', display: 'flex', flexDirection: 'column', background: colors.surface, fontFamily: font }}>
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: space.md, paddingTop: 62 }}>
          <div style={{ width: 52, height: 52, borderRadius: radius.pill, background: colors.brand, color: colors.onBrand, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 18, fontWeight: fw.semibold }}>LM</div>
          <div style={{ fontSize: 20, fontWeight: fw.semibold, color: colors.text }}>Oi de novo, Larissa</div>
          <div style={{ fontSize: 14, color: colors.textSecondary, textAlign: 'center', maxWidth: 230 }}>Use a digital para desbloquear o Dindim.</div>
          <div style={{ marginTop: space.lg, width: 96, height: 96, borderRadius: radius.pill, border: `2px solid ${colors.brand}`, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            <div style={{ width: 42, height: 50, borderRadius: '50% 50% 45% 45%', border: `2px solid ${colors.brand}`, position: 'relative' }}>
              <div style={{ position: 'absolute', inset: 8, borderRadius: '50% 50% 45% 45%', border: `2px solid ${colors.brand}`, opacity: 0.5 }} />
            </div>
          </div>
        </div>
        <div style={{ padding: `0 ${space.lg}px ${space.xl}px`, display: 'flex', flexDirection: 'column', gap: space.md }}>
          <div style={{ textAlign: 'center', fontSize: 14, fontWeight: fw.medium, color: colors.brand }}>Usar senha</div>
...
const recentTx = [
  ...
  { desc: 'Netflix', cat: 'Assinaturas', color: '#8A6D3B', amount: 3990, kind: 'expense' },
...
const catSpend = [
  ...
  { name: 'Assinaturas', color: '#8A6D3B', value: 3990 },
...
const accounts = [
  { name: 'Conta corrente', type: 'Conta corrente', color: '#B8860B', balance: 842035 },
  { name: 'Reserva de emergência', type: 'Poupança', color: '#00A868', balance: 406000 },
  { name: 'Cartão principal', type: 'Cartão de crédito', color: '#5A4A1F', balance: -127800 },
...
const expenseCats = [
  ...
  { name: 'Lazer', color: '#D10A8E' }, { name: 'Assinaturas', color: '#8A6D3B' },
  { name: 'Compras', color: '#0AB5B5' }, { name: 'Contas', color: '#5A4A1F' },
...
Object.assign(window, {
  ...
  LoginScreen, SignUpScreen, UnlockScreen, HomeScreen, HomeScreenDark, TransactionsScreen, AddTransactionScreen, AccountsScreen, GoalsScreen, CategoriesScreen, ProfileScreen,
});
